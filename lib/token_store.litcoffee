...
Adds to the store a new token and document 'id', and distinguishes between variant doc matches and normal name matches.
    ...
    hummingbird.TokenStore::add = (token, isVariant, docId) ->
      ...
      unless isVariant
        ...
        # variant token matches
        if @root[token]?['n']? and @root[token]['n'].indexOf(docId) is -1
          @root[token]['v'] ?= []
...
Number of documents associated with the given token
    ...
    hummingbird.TokenStore::count = (token) ->
      ...
      count = 0
      count += Object.keys(@root[token]['n']).length if Object.keys(@root[token]?['n']?)
      count += Object.keys(@root[token]['v']).length if Object.keys(@root[token]?['v']?)
      return count
